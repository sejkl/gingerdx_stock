...
check_installscript() {
	if [ -f "/tmp/.installscript" ] && [ $PROCEED -ne 0 ]; then
		if [ -f "$S/etc/force_backuptool" ] || [ -f "$C/force_backuptool" ]; then
...
get_custom_files() {
	local L
	if [ -f "$C/custom_backup_list.txt" ]; then
		[ ! -f $C/fixed_custom_backup_list.txt ] && tr -d '\r' < $C/custom_backup_list.txt > $C/fixed_custom_backup_list.txt
